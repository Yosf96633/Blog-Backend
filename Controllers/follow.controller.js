...
export const followUnfollow = async (req, res) => {
    ...
    try {
        ...
        if (!userFollow) {
            userFollow = new Follow_Model({ user: _id, followers: [], following: [] });
        }
        let targetUserFollow = await Follow_Model.findOne({ user: Tid });
        if (!targetUserFollow) {
            targetUserFollow = new Follow_Model({ user: Tid, followers: [], following: [] });
        }
...
export const getFollowers = async (req , res) => {
    try {
        const {_id} = req.user;
        const result = await Follow_Model.findOne({user:_id}).populate(`follower` , `name imageDetails.imageURL`)
        if(!result || result.follower.length===0){
            return res.status(200).json({message:`No followers found` , follower:[] , success:false})
        }
        return res.status(200).json({ message:`Followers found` , follower:result.follower , success:true})
    } catch (error) {
...
export const getFollowings = async (req , res) => {
    try {
        const {_id} = req.user;
        const result = await Follow_Model.findOne({user:_id}).populate(`following` , `name imageDetails.imageURL`)
        if(!result || result.following.length===0){
            return res.status(200).json({message:`No following found` , following:[] , success:false})
        }
        return res.status(200).json({ message:`Following found` , following:result.following , success:true})
    } catch (error) {
...
export const checkIfUserFollows = async (req, res) => {
   ...
   const Tid = req.params.id
    try {
        ...
        if (!userFollow) {
            return res.status(404).json({ message: "Follow record not found" , success:false});
        }
